...
exports.parseCodeContext = function(str, parentContext) {
  ...
  // class constructor
  } else if (/^\s*constructor\s*\([\s\S]*\)\s*{[\s\S]*}/.exec(str)) {
    return {
      type: 'constructor'
  ...
  // class method
  } else if (/^\s*([\w$]+)\s*\([\s\S]*\)\s*{[\s\S]*}/.exec(str)) {
    console.log('class method', RegExp.$0);
    return {
      type: 'method'
      , constructor: parentContext.name
      , cons: parentContext.name
  ...
  } else if (/^\s*([\w$.]+)\s*\.\s*prototype\s*\.\s*([\w$]+)\s*=\s*({[\s\S]+})/.exec(str)) {
    return {
        type: 'property'
      , constructor: RegExp.$1
      , cons: RegExp.$1
  ...
  // inline prototype
  } else if (/^\s*([\w$.]+)\s*\.\s*prototype\s*=\s*({[\s\S]+})/.exec(str)) {
    return {
      type: 'prototype'
      , constructor: RegExp.$1
      , cons: RegExp.$1
