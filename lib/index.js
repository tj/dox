...
var render = exports.render = function(str, file){
    ...
    for (var i = 0, len = parts.length; i < len; ++i) {
        ...
        // Ignored comment
            ...
            ++i;
            // haha ...
            if (isPrivate(part) && !showPrivate) continue;
            var part = part.replace(/^ *\* ?/gm, '');
            blocks.push({
               comment: markdown.parse(toMarkdown(escape(part))),
               code: koala.render('.js', parseMethod(escape(next), file)) 
            });
        }
    }
...
/**
 * Check if the given string of code appears to be private.
 *
 * @param {String} str
 * @return {Boolean}
 * @api private
...
/**
 * Parse method or function in the given code string.
 *
 * Examples:
 *
 *     "function foo(){}"
 *     // => "function <a href="">foo</a>(){}"
 *
 * @param {String} str
 * @param {String} file
 * @return {String}
...
function parseMethod(str, file) {
    return str
        .replace(/function *(\w+)/, 'function <a href="#' + file + '-func-$1">$1</a>');
}
